...
public class VerificarVinculoOS implements TarefaJava {
	...
	public void getContextualizaPrimeiroItem(BigDecimal solicitante, Map<String, Object> item) throws Exception {
		BigDecimal verificaFila = NativeSql.getBigDecimal("CODUSU", "TCSRUS", "CODUSU IN (186, 125) AND TIPO = 'F' AND CODUSUREL = ? ORDER BY CODUSU DESC", new Object[] { solicitante });
		BigDecimal filaSoftware = new BigDecimal(186);

		if (verificaFila != null && filaSoftware.compareTo(verificaFila) == 0) {
			item.put("CODUSU", filaSoftware);
			item.put("CODSERV", new BigDecimal(50506));
			item.put("CODSIT", BigDecimal.ONE);
		} else {
			item.put("CODUSU", new BigDecimal(46));
