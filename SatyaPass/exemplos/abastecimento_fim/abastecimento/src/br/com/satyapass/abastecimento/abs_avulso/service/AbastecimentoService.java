...
public class AbastecimentoService {
    ...
    public void gerenciarAbastecimento(BigDecimal codAbastecimento) throws Exception {
        ...
        if (StringUtils.isNotEmpty(abastecimentoVO.asBigDecimal("NUNOTA"))) {
            ...
            confirmarNota2(nroUnicoDaNota);
            abastecimentoRepository.atualizarNunotaDaRequisicao(codAbastecimento, nroUnicoDaNota);
        }

    }

    private void validarCampo(DynamicVO abastecimentoVO) throws Exception {
        BigDecimal codAbastecimento = abastecimentoVO.asBigDecimal("CODABAST");
        if (StringUtils.isEmpty(abastecimentoVO.asBigDecimal("CODPROD")))
            lancarErro("Produto", codAbastecimento);

        if (StringUtils.isEmpty(abastecimentoVO.asBigDecimal("CODVEICULO")))
            lancarErro("Veiculo", codAbastecimento);

        if (StringUtils.isEmpty(abastecimentoVO.asBigDecimal("CODPARC")))
            lancarErro("Parceiro", codAbastecimento);
    }

    private void lancarErro(String campo, BigDecimal nroAbastecimento) throws Exception {
        throw new Exception("Para gerar a requisição, é obrigatorio ter o "+ campo + ". Nro do Abastecimento: " + nroAbastecimento);
    }
