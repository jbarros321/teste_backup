...
public class DeParaService {
    ...
    public void gerenciarDePara(BigDecimal nroUnico, boolean isTransacaoManual) throws Exception {
        ...
        if (!itensVO.isEmpty()){
            for (DynamicVO itemVO: itensVO){
                if (isTransacaoManual)
                    this.deParaRepository.incluirProdutoTM(itemVO.asString("PRODUTO").trim());
                else
