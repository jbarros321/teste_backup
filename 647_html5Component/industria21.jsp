...
<body>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control" id="dataIni" value="2025-10-01" onchange="updateDateDisplay('dataIni', 'dataIniDisplay')">
                    <small class="text-muted" id="dataIniDisplay">01/10/2025</small>
                </div>
                <div class="mb-3">
                    <label for="dataFin" class="form-label">Data Final</label>
                    <input type="date" class="form-control" id="dataFin" value="2025-10-21" onchange="updateDateDisplay('dataFin', 'dataFinDisplay')">
                    <small class="text-muted" id="dataFinDisplay">21/10/2025</small>
                </div>
    ...
    <script>
        ...
        // Função para atualizar exibição visual de data em formato brasileiro
        ...
        async function showDetails(kpi, cd, month, value) {
            ...
            if (kpi === 'custoCD') {
                ...
                try {
                    const dataIni = document.getElementById('dataIni').value;
                    const dataFin = document.getElementById('dataFin').value;
                    title = `${kpiTitles[kpi]} - Detalhamento (período: ${formatDate(dataIni)} a ${formatDate(dataFin)})`;
        ...
        async function applyFilters() {
            ...
            try {
                ...
                // Atualizar exibição visual das datas
                updateDateDisplay('dataIni', 'dataIniDisplay');
                updateDateDisplay('dataFin', 'dataFinDisplay');
        ...
        function createLineChart(canvasId, chartData, kpi) {
            ...
            const beginAtZero = kpi !== 'qualidade' && kpi !== 'formulacao' && kpi !== 'envase' && kpi !== 'volumeRealizado' && kpi !== 'ocupacaoProducao' && kpi !== 'manutencaoPreventiva' && kpi !== 'manutencaoCorretiva' && kpi !== 'custoCD';
            const suggestedMax = (kpi === 'qualidade' || kpi === 'formulacao' || kpi === 'envase' || kpi === 'volumeRealizado' || kpi === 'ocupacaoProducao' || kpi === 'manutencaoPreventiva' || kpi === 'manutencaoCorretiva' || kpi === 'custoCD') ? 100 : undefined;
        ...
        document.addEventListener('DOMContentLoaded', async function() {
            // Inicializar exibição visual das datas
            updateDateDisplay('dataIni', 'dataIniDisplay');
            updateDateDisplay('dataFin', 'dataFinDisplay');
