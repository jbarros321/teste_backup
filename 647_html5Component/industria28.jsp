...
<body>
    ...
    <script>
        ...
        /** Preenche data inicial/final com o primeiro e último dia do mês corrente e alinha variáveis de período. */
        function aplicarPeriodoMesAtualNosFiltros() {
            const hoje = new Date();
            const y = hoje.getFullYear();
            const m = hoje.getMonth();
            const pad = function(n) { return String(n).padStart(2, '0'); };
            const primeiroDia = y + '-' + pad(m + 1) + '-01';
            const ultimoDiaDate = new Date(y, m + 1, 0);
            const ultimoDia = y + '-' + pad(m + 1) + '-' + pad(ultimoDiaDate.getDate());
            var elIni = document.getElementById('dataIni');
            var elFin = document.getElementById('dataFin');
            if (elIni) elIni.value = primeiroDia;
            if (elFin) elFin.value = ultimoDia;
            selectedPeriod = { start: primeiroDia, end: ultimoDia };
            filteredIndices = [m];
            filteredLabels = [months[m]];
        }
            ...
            try {
                ...
                if (result && result.length > 0) {
                    result.forEach((empresa) => {
                        ...
                        const cod = String(empresa.CODEMP != null ? empresa.CODEMP : empresa.codemp);
                        if (cod === '1') {
...
ORDER BY CB.CODEMP, CB.DTNEG, CB.NUNOTA, CB.CODPROD`;
        ...
        document.addEventListener('DOMContentLoaded', async function() {
            aplicarPeriodoMesAtualNosFiltros();
            updateSubtitlePeriodo();
