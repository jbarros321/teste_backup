...
<body>
            ...
            <div class="card">
                <div class="card-body p-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control form-control-sm" id="dataIni" placeholder="DD/MM/AAAA" maxlength="10" inputmode="numeric">
                    </div>
                    <div class="mb-3">
                        <label for="dataFin" class="form-label">Data final</label>
                        <input type="text" class="form-control form-control-sm" id="dataFin" placeholder="DD/MM/AAAA" maxlength="10" inputmode="numeric">
                    </div>
                    <div class="mb-3">
    ...
    <script>
        ...
        function buildFilteredPeriods(dataIni, dataFin) {
            const start = parseDateFromFilter(dataIni);
            const end = parseDateFromFilter(dataFin);
        ...
        // Função para normalizar data do filtro para DD/MM/YYYY
        ...
        // Função para transformar data do filtro em Date válido
        function parseDateFromFilter(dateStr) {
            const br = normalizeDateBR(dateStr);
        ...
        // Função para converter data BR (DD/MM/YYYY) para DD-MM-YYYY (SQL)
        ...
        /** Define dataIni/dataFin como primeiro e último dia do mês corrente (DD/MM/YYYY). */
        function aplicarDatasMesAtual() {
            const now = new Date();
            const y = now.getFullYear();
            const m = now.getMonth();
            const pad = function(n) { return String(n).padStart(2, '0'); };
            const ini = '01/' + pad(m + 1) + '/' + y;
            const ultimoDia = new Date(y, m + 1, 0).getDate();
            const fin = pad(ultimoDia) + '/' + pad(m + 1) + '/' + y;
...
ORDER BY CB.CODEMP, CB.DTNEG, CB.NUNOTA, CB.CODPROD`;
        ...
        async function carregarDetalhesCustoEntrega(cd, month, value) {
            try {
                ...
                if (month != null && month !== '') {
                    const year = parseDateFromFilter(dataIni).getFullYear();
        ...
        function updateSubtitlePeriodo() {
            ...
            if (!el) return;
            if (dataIni && dataFin) {
                el.textContent = 'Gerenciamento de processos — ' + formatDate(dataIni) + ' a ' + formatDate(dataFin);
            } else {
